...
public class Player2NpcCharacterCardWidget extends AbstractWidget {
    ...
    static void renderVitals(GuiGraphics graphics, int right, int bottom, Player2NpcHubScreen.CompanionVitals vitals) {
        ...
        for (int i = 0; i < hearts; i++) {
            ...
            int y = bottom - 20;
            graphics.blit(GUI_ICONS_LOCATION, x, y, HEART_CONTAINER_U, HEART_V, 9, 9);
            if (i * 2 + 1 < health) {
                graphics.blit(GUI_ICONS_LOCATION, x, y, HEART_FULL_U, HEART_V, 9, 9);
            } else if (i * 2 + 1 == health) {
                graphics.blit(GUI_ICONS_LOCATION, x, y, HEART_HALF_U, HEART_V, 9, 9);
            }
        }
        for (int i = 0; i < 10; i++) {
            int x = right - i * 8 - 9;
            int y = bottom - 10;
            graphics.blit(GUI_ICONS_LOCATION, x, y, FOOD_EMPTY_U, FOOD_V, 9, 9);
            if (i * 2 + 1 < food) {
                graphics.blit(GUI_ICONS_LOCATION, x, y, FOOD_FULL_U, FOOD_V, 9, 9);
            } else if (i * 2 + 1 == food) {
                graphics.blit(GUI_ICONS_LOCATION, x, y, FOOD_HALF_U, FOOD_V, 9, 9);
            }
        }
